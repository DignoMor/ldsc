...
def make_annot_files(args, beds_for_annot, names_for_annot):
    ...
    for name_for_annot, bed_for_annot in zip(names_for_annot, beds_for_annot):
        annotbed = bimbed.intersect(bed_for_annot)
        bp = [x.start + 1 for x in annotbed]
        df_int = pd.DataFrame({'BP': bp, name_for_annot:1})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bed-file', type=str, action="append", 
                        help='the UCSC bed file with the regions that make up your annotation')
    parser.add_argument('--annot-name', type=str, action="append", 
                        help='Name for annotation for the bed file.')
    ...
    if args.gene_set_file is not None:
        ...
        for bed_file in args.bed_file:
            bed_for_annot = BedTool(bed_file).sort()
            if not args.nomerge:
                bed_for_annot = bed_for_annot.merge()
            bed_list.append(bed_for_annot)

    make_annot_files(args, bed_list, args.annot_name)
